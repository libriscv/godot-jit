...
""")
    ...
    var nested_script = load("res://tests/ugd/nested_classes.ugd")
    var nested = nested_script.DayStrip.new(13)
    check(nested is Control and nested.initial == 13, "GDScript constructs a compiled nested class")
    check(nested.get_script() == nested_script.DayStrip, "Nested class resource identity is shared")
    check(nested_script.DayStrip.answer() == 42, "GDScript calls nested static methods")
    nested.free()
