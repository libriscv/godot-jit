...
class UnsafeGDScriptHighlightRules {
	struct Region {
		String start;
		String end;
		Color color;
...
class UnsafeGDScriptCodeHighlighter : public SyntaxHighlighter {
	GDCLASS(UnsafeGDScriptCodeHighlighter, SyntaxHighlighter);

	mutable UnsafeGDScriptHighlightRules rules;
	Ref<Script> edited;
...
// Answers "UnsafeGDScript" so ScriptEditor selects it.
class UnsafeGDScriptSyntaxHighlighter : public EditorSyntaxHighlighter {
	GDCLASS(UnsafeGDScriptSyntaxHighlighter, EditorSyntaxHighlighter);
...
public:
	virtual String _get_name() const override { return "UnsafeGDScript"; }
