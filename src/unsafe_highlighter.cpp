// Adapted from godot-sandbox SafeGDScript; both use the same language syntax.
...
void UnsafeGDScriptHighlightRules::update(TextEdit *p_text_edit, const Ref<Script> &p_edited) {
	keywords.clear();
	...
	ScriptLanguageExtension *language = unsafe_language();
...
int UnsafeGDScriptHighlightRules::match_region(const String &p_text, int p_from) const {
	const int length = p_text.length();
...
int UnsafeGDScriptHighlightRules::scan_line(const String &p_text, int p_entry_region, Dictionary *r_map) const {
	const int length = p_text.length();
	int i = 0;
...
int UnsafeGDScriptHighlightRules::region_at_line_start(TextEdit *p_text_edit, int p_line) {
...
Dictionary UnsafeGDScriptHighlightRules::highlight_line(TextEdit *p_text_edit, int p_line) {
...
Dictionary UnsafeGDScriptCodeHighlighter::_get_line_syntax_highlighting(int32_t p_line) const {
	return rules.highlight_line(get_text_edit(), p_line);
}

void UnsafeGDScriptCodeHighlighter::_clear_highlighting_cache() {
	rules.clear();
}

void UnsafeGDScriptCodeHighlighter::_bind_methods() {
	ClassDB::bind_method(D_METHOD("set_edited_script", "script"), &UnsafeGDScriptCodeHighlighter::set_edited_script);
	ClassDB::bind_method(D_METHOD("get_edited_script"), &UnsafeGDScriptCodeHighlighter::get_edited_script);
...
void UnsafeGDScriptCodeHighlighter::set_edited_script(const Ref<Script> &p_script) {
	edited = p_script;
	clear_highlighting_cache();
	update_cache();
}

void UnsafeGDScriptCodeHighlighter::_update_cache() {
	rules.update(get_text_edit(), edited);
}

PackedStringArray UnsafeGDScriptSyntaxHighlighter::_get_supported_languages() const {
	PackedStringArray languages;
	languages.push_back("UnsafeGDScript");
	return languages;
}

Ref<EditorSyntaxHighlighter> UnsafeGDScriptSyntaxHighlighter::_create() const {
	Ref<UnsafeGDScriptSyntaxHighlighter> highlighter;
	highlighter.instantiate();
	return highlighter;
}

Dictionary UnsafeGDScriptSyntaxHighlighter::_get_line_syntax_highlighting(int32_t p_line) const {
	return rules.highlight_line(get_text_edit(), p_line);
}

void UnsafeGDScriptSyntaxHighlighter::_clear_highlighting_cache() {
	rules.clear();
}

void UnsafeGDScriptSyntaxHighlighter::_update_cache() {
